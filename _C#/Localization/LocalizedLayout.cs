...
namespace NeonKatana
{
    ...
    public class LocalizedLayout : MonoBehaviour
    {
        ...
        void Awake()
        {
            element = (RectTransform)transform;
        ...
        void Apply()
        {
            if (!TryGetHome(out Vector2 home)) return;

            // An element that something else is also moving — an entrance, a drift — has an owner
            // for its transform, and writing past it would only be undone on the next frame. Its
            // home is a base to move around, not a final position.
            if (driver == null) TryGetComponent(out driver);

            if (driver != null) driver.SetBasePosition(home);
            else element.anchoredPosition = home;
        }
